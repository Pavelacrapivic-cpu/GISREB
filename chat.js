...
class GisrebChat {
    ...
    async connectWebSocket() {
        // ЯВНО указываем адрес ВАШЕГО сервера на TimeWeb
        const backendHost = 'gisreb.ru'; // Ваш домен

        // Используем wss:// (безопасный) для HTTPS или ws:// для HTTP
        const protocol = 'wss://'; // Обязательно для Telegram Web App
        // const protocol = 'ws://'; // Только для локального тестирования

        const wsUrl = `${protocol}${backendHost}/ws`;

        console.log('🔄 Подключаемся к WebSocket:', wsUrl); // Для отладки
        ...
        const messageHTML = `
            <div class="message ${sender}-message" id="${messageId}">
                <div class="message-sender">
                    ${sender === 'user' ? 'Вы' : 'ГИС РЭБ Ассистент'}
    ...
    showTypingIndicator() {
        ...
        const typingHTML = `
            <div class="message bot-message" id="typing-indicator">
                <div class="message-sender">ГИС РЭБ Ассистент</div>
